        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => navigate("/appointment/billing", {
              state: {
                prefillAppointment: bookedApt,
                autoGenerateBill: true,
              }
            })}
            className="flex-1 py-3 bg-amber-500 text-white rounded-xl font-bold text-sm hover:bg-amber-600 transition flex items-center justify-center gap-2"
          >
          ...
          <div>
            ...
            <div className="relative">
              ...
              {loadingSlots ? (
                ...
                <select
                  className="apt-input py-3 text-sm"
                  style={{ paddingLeft: "2.5rem" }}
                  value={bookingSlot?._id || ""}
                  onChange={(e) => {
                    const s = slots.find(sl => sl._id === e.target.value);
                    setBookingSlot(s);
                  }}
                >
                  <option value="">Walk-in (no slot)</option>
                  {slots.filter(s => s.status !== "booked").map(s => (
                    <option key={s._id} value={s._id}>{s.startTime}</option>
                  ))}
                </select>
              )}
            </div>
            {slots.length > 0 && (
              <p className="text-[10px] text-gray-400 mt-1">
                {slots.filter(s => s.status !== "booked").length} slots available
              </p>
            )}
          </div>
        </div>
